...
1. **Red (Fail)**: `@Trace SPEC-ID`를 포함한 테스트 코드를 작성합니다.
   - **Scenario-driven Architecture (추천)**: 하나의 유저 플로우(User Flow)를 하나의 테스트 메서드(`@Test`)로 1:1 매핑합니다. 
   - **Modularization**: 테스트 메서드 내부는 "Given-When-Then" 단계별로 모듈화된 명령(또는 주석 블록)으로 구성하여, 기획서의 흐름을 그대로 복제합니다. 이렇게 하면 RTM과 테스트 간의 매핑이 매우 직관적이고 견고해집니다.
